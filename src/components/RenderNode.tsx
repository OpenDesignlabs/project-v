...
export const RenderNode: React.FC<RenderNodeProps> = ({ elementId }) => {
    ...
    const handlePointerDown = (e: React.PointerEvent) => {
        // 1. Always stop propagation so we don't select the parent container instead
        e.stopPropagation();

        if (isLocked || previewMode) {
            if (previewMode && element.events?.onClick && element.events.onClick.type === 'NAVIGATE') {
                alert(`Navigating to: ${element.events.onClick.payload}`);
            }
            return;
        }

        // 2. Select Immediately (Fixes "Not selected when I click")
        setSelectedId(elementId);

        // 3. Enter Edit Mode if using Type Tool
        if (activeTool === 'type' && (element.type === 'text' || element.type === 'button')) {
            setIsEditing(true);
            return;
        }

        // 4. Handle Moving (Standard Drag)
        ...
        setInteraction({
            ...
            startRect: {
                ...
                width: parseFloat(style.width?.toString() || '100'),
                height: parseFloat(style.height?.toString() || '50')
            }
        });
    };
    ...
    const handleContentEdit = (e: React.FormEvent<HTMLDivElement>) => {
        ...
        // Deep copy to ensure History/Undo works
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
        // CRITICAL FIX: Stop Backspace/Delete from bubbling to the global "Delete Element" listener
        e.stopPropagation();
    ...
    const handleDrop = (e: React.DragEvent) => {
        ...
        if (dragData.type === 'NEW') {
            ...
            if (tpl) {
                ...
                if (newNodes[newRootId]?.props) {
                    newNodes[newRootId].props.style = { ...(newNodes[newRootId].props.style || {}), ...newStyle };
                }
            }
        }
        ...
        if (newRootId) {
            ...
            updatedProject[elementId] = {
                ...updatedProject[elementId],
                children: [...(updatedProject[elementId].children || []), newRootId]
            };
        ...
        content = (
            <>
                {element.type === 'canvas' && !previewMode && (
                    <div className="absolute top-0 right-0 bg-blue-100 text-blue-600 text-[10px] px-3 py-1 rounded-bl-lg pointer-events-none z-0 font-medium">
                        {element.props.layoutMode === 'flex' ? 'Auto Layout' : 'Artboard'}
                    </div>
                )}
    ...
    return (
        ...
        >
            {/* Component Name Badge (Only when selected) */}
